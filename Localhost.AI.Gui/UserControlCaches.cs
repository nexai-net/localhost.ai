...
namespace Localhost.AI.Gui
{
    ...
    public partial class UserControlCaches : UserControl
    {
        private SessionManager _session;
        private Config _config;
        private List<Cache> _caches = new List<Cache>();

        public UserControlCaches()
        {
            InitializeComponent();
        ...
        private void LoadData(string searchTerm)
        {
            ...
            try
            {
                _caches = _session.CacheSearchByValue(searchTerm);
                dataGrid.DataSource = _caches;
            }
            catch (Exception ex)
            {
        ...
        {
            var selectedCache = dataGrid.Rows[e.RowIndex].DataBoundItem as Cache;
            if (selectedCache != null)
            {
                var editForm = new CacheEditForm(selectedCache);
                editForm.Show();
            }
        }
    }
